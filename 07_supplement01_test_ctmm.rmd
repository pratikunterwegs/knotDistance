...
fig_sld_ctmm_comapre <- ggplot(data %>% filter(est > 0, sld > 0,
                                               tag %in% sample(unique(data$tag))[1:15]))+
  ...
  labs(x = "straight line displacement speed (m/s)",
       ...
       caption = "shows 10% of all data at 30s scale")+
  coord_fixed(ratio=0.8)+
  scale_colour_scico(palette = "batlow")+
  facet_wrap(~tag, labeller = label_both, nrow = 5)+
  theme_bw()+
  theme(panel.grid.minor = element_blank(),
        axis.text = element_text(size = 8))

ggsave(fig_sld_ctmm_comapre, 
       filename = "figs/fig_sld_ctmm.png", height = 10, width = 10, dpi=300)
dev.off()
```
