...
# read in griend
library(sf)
griend <- st_read("data/griend_polygon/") 
...
data <- purrr::map_df(speed_files, function(df){
  a <- fread(df)
  setDF(a)
  a <- a %>% select(id, tide_number, x,y,est) %>% 
...
fig_speed_example <- ggplot(data)+
  geom_sf(data = griend)+
  geom_path(aes(x, y, col = est, group = tide_number, size = est > 5), alpha = 0.2)+
  geom_point(aes(x, y, col = est, group = tide_number, size = est > 5), alpha = 0.5)+
...
fig_speed_histogram <- ggplot(data)+
  geom_histogram(aes(x = est, y = stat(density*0.05)), binwidth = 0.05)+
  facet_wrap(~tag, labeller = label_both)+
  scale_x_log10()+
  coord_cartesian(xlim = c(0.1,20))+
  theme_few()+
  labs(x = "speed estimate (m/s) log scale", y = "proportion")

# save figure
ggsave(fig_speed_histogram, filename = "figs/fig_speed_histogram.png", height = 5, width =8, dpi=300)
dev.off()
