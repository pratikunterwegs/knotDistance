...
{
  data <- fread("data/mod_output/speed_estimates_2018.csv")
  data <- setDF(data) %>% as_tibble() %>% 
    rename(mean = est) %>% 
...
fig_compare_ctmm <- ggplot(data)+
  geom_line(aes(x = scale, y = speed, group = interaction(id, tide_number),
