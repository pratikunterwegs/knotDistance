...
pmap(data_to_test, function(scales, data_files){
  ...
  if(!data_name %in% files_on_prg){
    ...
    scp_upload(s, data_files, to = "data/watlas/")
  }
  ...
  # make job file
  {
    ...
    # rename job
    shebang[2] <- glue('#SBATCH --job-name=ctmm_{data_files}')
    
    text <- glue('Rscript code/code_test_ctmm_scale.r {data_files} {scales} ')
    jobfile <- glue('code/job_ctmm_{data_files}_{scales}.sh')
